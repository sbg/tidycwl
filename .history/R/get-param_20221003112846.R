...
get_steps_version <- function(steps) {
  if (!is.null(steps$run)) {
    ...
    if (is_cwl_dict(steps)) {
      version <- steps[[run_str_gbl]][["sbg:toolkitVersion"]]
    } else if (is_cwl_list(steps)) {
      version <- unlist(get_el_from_list(get_el_from_list(steps, run_str_gbl), "sbg:toolkitVersion"))
    } else {
      stop("`steps` is not a proper dict or list")
    }
  }
...
get_steps_revision <- function(steps) {
  ...
  if (is.null(param)) {
    if (ver_1_0_str_gbl %in% get_el_from_list(get_el_from_list(steps, run_str_gbl), cwl_ver_str_gbl)) param <- "sbg:latestRevision"
    if (ver_1_1_str_gbl %in% get_el_from_list(get_el_from_list(steps, run_str_gbl), cwl_ver_str_gbl)) param <- "sbg:latestRevision"
    if (ver_1_2_str_gbl %in% get_el_from_list(get_el_from_list(steps, run_str_gbl), cwl_ver_str_gbl)) param <- "sbg:latestRevision"
    if (sbg_draft_str_gbl %in% get_el_from_list(get_el_from_list(steps, run_str_gbl), cwl_ver_str_gbl)) param <- "sbg:revision"
  }
